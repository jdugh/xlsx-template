...
//
    ...
    */
    ...
    //must point to correct <Relationship> 'Id' in xl\_rels\workbook.xml.rels
      var self = this;
      var order = ['worksheet', 'theme', 'styles','sharedStrings', 'customXml'];
    ...
    Workbook.prototype.substitute = function(sheetName, substitutions) {
        ...
        sheetData.findall("row").forEach(function(row) {
            ...
            var cells = [],
                cellsInserted = 0,
                newTableRows = [],
                cellsForsubstituteTable = []; //Contains all the row cells when substitute tables
            ...
            if(newTableRows.length > 0) {
                ...
                //Filtre all the cellsForsubstituteTable cell with the 'row' cell
                //Don't can make a filter instead of this ugly foreach ?
                var isSubititueCell = false;
                var cellsOverTable = []; //In this array, I have just the cell where are not in row.findall("c")
                row.findall("c").forEach(function(rowCell) {
                    isSubititueCell = false;
                    cellsForsubstituteTable.forEach(function(subtitueCell){
                        if(rowCell == subtitueCell){
                            isSubititueCell = true;
                        }
                    });
                    if (isSubititueCell == false){
                        cellsOverTable.push(rowCell)
                    }
                });
    ...
    Workbook.prototype.substituteImage = function (cell, string, placeholder, substitution, drawing) {
        ...
        try {
            ...
            // console.log("Error on reading image object : ", substitution)
            return false;
        }
        if( substitution == null){
            // console.log("Cannot read image : ", substitution)
            return false;
        }
        ...
        try {
            ...
            // console.log("Error on reading image size object : ", substitution)
            return false;
        }
    ...
    Workbook.prototype.columnWidthToEMUs = function (width) {
        ...
        return Math.round(width * 66691.28205);
    }
